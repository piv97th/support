...
<?php
	...
	{
		require('blocks/connect.php');
		$result = $conn->query('SELECT timetable_meeting.*, group_1.id as `id_group`, group_1.cipher_group FROM timetable_meeting INNER JOIN group_1 ON timetable_meeting.id = group_1.id_meeting_se_fk WHERE timetable_meeting.id_commission_fk IN (SELECT id FROM commission WHERE id IN (SELECT id_commission_fk FROM curation_event WHERE id IN (SELECT id_curevent_fk FROM user WHERE id = '.$arr_1.')))');
	...
	<div class="container" id="content">    
		<div class="row content">
			<div class="col-sm text-left"> 
				<div>
					<legend>Проведение госэкзамена</legend>
					Заседание
